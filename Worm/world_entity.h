...
inline int form_visual(BYTE*& blc, WorldItem block_, World world_, ENetPeer* peer, bool case_ = true) {
	...
	switch (itemDefs.at(block_.foreground).blockType) {
		...
		case BlockTypes::GIVING_TREE: {
			BYTE btype = 0x1C, c = block_.int_data, a = 0;
			long long time_ = time(nullptr);
			uint16_t b = (uint16_t)(block_.block_time - time_ <= 0 ? -1 : 10800 - (block_.block_time - time_));
			memset(blc + 8, 0, 7);
			memcpy(blc + 8, &btype, 1);
			memcpy(blc + 8 + 1, &a, 1);
			memcpy(blc + 8 + 2, &b, 4);
		...
		case BlockTypes::GEIGER_CHARGER: {
			...
			memcpy(blc + 8, &btype, 1);
			long long time_ = time(nullptr);
			uint16_t sk = (uint16_t)(block_.block_time - time_ <= 0 ? 3600 : 3600 - (block_.block_time - time_));
			memcpy(blc + 9, &sk, 2);
			if (case_) {
				blc += 5;
				return 5;
...
inline void sendWorld(ENetPeer* peer, World* worldInfo) {
	...
	int count = int(worldInfo->droppedItems.size()), last_uid = int(worldInfo->droppedItems.size());
	memcpy(blc, &count, 4);
