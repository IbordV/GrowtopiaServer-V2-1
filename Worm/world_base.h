		...
		if (pInfo(peer)->haveGrowId and pInfo(peer)->inGame and pInfo(peer)->passed_data and not pInfo(peer)->tankIDName.empty() and not pInfo(peer)->lobby_guest_acc and pInfo(peer) != NULL) {
			...
			if (not w_.owner_name.empty()) {
				try {
					...
					Send_::console_msg(peer, "`#[`w" + w_.name + "`$ World Locked `oby " + j["displayNameBackup"].get<string>() + "" + status + "");
					j.clear();
				}
				catch (exception& e) {
					cout << e.what() << endl;
...
inline void join_setting(ENetPeer* peer, string act_, bool door_gateway = false, int x_to = -1, int y_to = -1) {
	try {
		bool is_valid = true;
		...
		if (pInfo(peer)->haveGrowId and pInfo(peer)->inGame and pInfo(peer)->passed_data and not pInfo(peer)->tankIDName.empty()) {
			if (not door_gateway) {
				using namespace chrono;
				if (pInfo(peer)->currentWorld == act_) return;
				if (pInfo(peer)->world_join_left + 1500 > (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count()) {
					gamepacket_t p;
					p.Insert("OnFailedToEnterWorld");
					p.CreatePacket(peer);
...
inline void dropitem(ENetPeer* peer, World* world, int netID, float x, float y, int item_, int c_t, BYTE bapakkaupuki) {
	try {
		...
		if (item_ != 112) {
			...
			for (int i = 0; i < (int)world->droppedItems.size(); i++) {
				if ((int)(x / 32) == (int)(world->droppedItems.at(i).x / 32) and (int)(y / 32) == (int)(world->droppedItems.at(i).y / 32)) {
					if (world->droppedItems.at(i).count < 200 and world->droppedItems.at(i).id == item_ and world->droppedItems.at(i).count + c_t <= 200) {
...
inline void saveitem_drop(ENetPeer* peer, World* world, int itemId, int Quantity, float x, float y) {
	try {
		...
		if (not valid) {
			...
			i_.itemID = itemId;
			inventory->items.push_back(i_);
		} else {
			int c_inv = 0;
			for (int i = 0; i < (int)pInfo(peer)->inventory.items.size(); i++) {
				if (pInfo(peer)->inventory.items.at(i).itemID == itemId) {
					c_inv = (unsigned int)pInfo(peer)->inventory.items.at(i).itemCount;
					if (c_inv < 0) {
						c_inv = fix_ct((unsigned int)pInfo(peer)->inventory.items.at(i).itemCount);
					}
				}
			}
			...
			if (plusItem > 200) {
				...
				inventory->items.at(idx).itemCount = c_inv + Quantity;
			}
		}
	}
	catch (out_of_range& e) {
